...
class ImagePatch:
    ...
    def __init__(self, image: Union[Image.Image, torch.Tensor, np.ndarray], left: int = None, lower: int = None,
        ...
        if isinstance(image, Image.Image):
            ...
            image = image / 255
        # if config.load_models.llava:
        self.llava_image = image
    ...
    def get_pos(self):
        print(self.horizontal_center, self.vertical_center)
    ...
    def simple_query(self, question: str):
        ...
        """
        """Change the model here to use different VLM model"""
        if config.load_models.blip:
            return self.forward('blip', self.cropped_image, question, task='qa')
        if config.load_models.instructblip:
            return self.forward('instructblip', self.llava_image, question, task='qa')
        
        return self.forward('llava', self.llava_image, question, task='qa')
    ...
    def __repr__(self):
        return "ImagePatch({}, {}, {}, {}, {})".format(self.cropped_image,self.left, self.lower, self.right, self.upper).replace('tensor(', 'torch.tensor(') 
...
def direction_vector(direction_str: str) -> tuple[float, float]:
    """Converts a vector that is a str '(a, b)' to tuple(a, b)"""
    print(direction_str)
    first_float = float(direction_str.split('(')[1].split(',')[0])
    second_float = float(direction_str.split(')')[0].split(',')[1])
    answer = tuple([first_float, second_float])
    return answer
...
def recursive_query(parameters, question: str, start_index=0):
    """Returns the answer to a complicated question asked about the image. 
    Parameters
    -------
    question : str
...
def get_text_answer(info, question):
    print("get_text_answer")
    prompt = "\
    Given a question asked about an image/video, a dict of information related to the question, \
    answer the question using one brief sentence.\n"
    ...
    prompt_final = "\n\nAnswer:"
    full_prompt = prompt + prompt_question + prompt_info + prompt_final 
    print(full_prompt)
